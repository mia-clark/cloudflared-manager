...
export const DEFAULT_BRANDING: Branding = {
  ...
  app_subtitle: 'Cloudflare Tunnel 管理面板',
  html_title: 'Cloudflared 隧道管理器 · Cloudflare Tunnel 管理面板',
};

declare global {
  interface Window {
    // 由 daemon 注入 index.html 的引导脚本，供首帧同步读取实现零闪。
    __CFD_BRANDING__?: Partial<Branding>;
  }
}

// 同步读取 daemon 注入的引导品牌；缺失（如 vite dev 无注入）则回退默认。
export function readBootstrapBranding(): Branding {
  const b = typeof window !== 'undefined' ? window.__CFD_BRANDING__ : undefined;
  return {
